...
public class ClienteController {
	...
	public ResponseEntity<Cliente> buscar(@Valid @PathVariable Long id) {
		...
		/*
		 * if (clienteOpt.isPresent()) { return ResponseEntity.ok(clienteOpt.get()); }
		 * return ResponseEntity.notFound().build();
		 */
		return clienteOpt.isPresent() ? ResponseEntity.ok(clienteOpt.get()) : ResponseEntity.notFound().build();
	}
